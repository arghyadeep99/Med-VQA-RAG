...
df = pd.read_csv("./filtered_datasets_deepeval/filtered_llava_preds_slake_baseline_synced.csv")
df = df.head(10)
df_gt = pd.read_csv("./Slake_Chest.csv")

CHUNK_SIZE = 10
CSV_PATH = "deepeval_results_multimodal/deepeval_results_llava_slake_baseline.csv"
...
img_path_lookup = df_gt.set_index("qid")["img_name"].to_dict()
...
def to_cases(df: pd.DataFrame, metric_name: str, answer_col: str = "model_output", is_baseline=True) -> (
    ...
    for _, row in df.iterrows():
        ...
        # img_path = img_path.replace("\\", "/").split("/", 1)[1]
        img_path_local = "D:/NLP685Project/Slake_Chest_Images/" + img_path
...
for idx in range(0, total, CHUNK_SIZE):
    ...
    try:
        ...
        multimodal_answer_relevancy = evaluate(
            ...
            metrics=[multimodal_answer_relevancy_metric],
