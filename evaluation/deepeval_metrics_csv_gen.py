...
df_enhanced = pd.read_csv("./llava_preds_vqa_rad_graphrag.csv")
df_gt = pd.read_csv("./VQA_RAD_Chest_Data.csv")

CHUNK_SIZE = 5
CSV_PATH = "structured_evaluation_results_llava_rad_vqa_rad_graphrag_full.csv"
...
def to_cases(df: pd.DataFrame, metric_name: str, answer_col: str = "model_output", is_baseline=True) -> list[
    LLMTestCase]:
    cases = []
...
answer_relevancy_metric = AnswerRelevancyMetric(threshold=0.5, model="gpt-4.1-nano")
...
g_eval_metric = GEval(
    ...
    evaluation_steps=[
        ...
        "Penalize omissions of key medical details",
...
for idx in range(0, total, CHUNK_SIZE):
    ...
    df_enhanced_chunk = df_enhanced.iloc[idx:idx + CHUNK_SIZE]
    ...
    try:
        ...
        enhanced_answer_relevancy_cases = to_cases(df_enhanced_chunk, "AnswerRelevancy", is_baseline=False)
        enhanced_answer_relevancy = evaluate(
            test_cases=enhanced_answer_relevancy_cases,
            metrics=[answer_relevancy_metric]
        )
        all_results += collect_results(enhanced_answer_relevancy, "enhanced", "AnswerRelevancy")
    except Exception as e:
    ...
    try:
        # Enhanced GEval
        enhanced_g_eval_cases = to_cases(df_enhanced_chunk, "GEval", is_baseline=False)
        enhanced_g_eval = evaluate(
            test_cases=enhanced_g_eval_cases,
            metrics=[g_eval_metric]
        )
        all_results += collect_results(enhanced_g_eval, "enhanced", "GEval")
    except Exception as e:
